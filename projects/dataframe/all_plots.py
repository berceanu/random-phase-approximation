...
def plot_series(ax, table, column, temperature, neutron_number, counter, label, annotation,
                plot_type="linear-linear", energy_interval=(0.1, 10), ylabel=None):  # "$R$ %s" % units[column]
...
def plot_table(column, table, include_talys=False, talys_column=None, plot_type="linear-linear", energy_interval=(0.1, 10), ylabel=None):   # "$R$ %s" % units[column]
    for iso in isotopes:
        ...
        for j, T in enumerate(temperatures):
            plot_series(ax=ax, table=table, column=column, temperature=T, neutron_number=iso,
                        counter=j, label="%s MeV" % T, annotation="N = %s" % iso, plot_type=plot_type, energy_interval=energy_interval, ylabel=ylabel)
            if T == 0 and include_talys:
                plot_series(ax=ax, table=table, column=talys_column, temperature=T, neutron_number=iso,
                            counter=niso + j, label="TALYS %s MeV" % T, annotation="N = %s" % iso, plot_type=plot_type, energy_interval=energy_interval, ylabel=ylabel)
    ...
    for T in temperatures:
        ...
        for j, iso in enumerate(isotopes):
            plot_series(ax=ax, table=table, column=column, temperature=T, neutron_number=iso,
                        counter=j, label="%s" % iso, annotation="T = %s MeV" % T, plot_type=plot_type, energy_interval=energy_interval, ylabel=ylabel)
            if T == 0 and include_talys:
                plot_series(ax=ax, table=table, column=talys_column, temperature=T, neutron_number=iso,
                            counter=niso + j, label="TALYS %s" % iso, annotation="T = %s MeV" % T, plot_type=plot_type, energy_interval=energy_interval, ylabel=ylabel)
        fig.set_size_inches(width, height)
...
def main():
    ...
    plot_table(column="strength_function_fm", talys_column="tabulated_strength_function_fm", table=table, include_talys=True, plot_type="log-linear", energy_interval=(0, 20), ylabel="$R$ %s" % units["strength_function_fm"])
    # plot_table(column="strength_function_mb", talys_column="tabulated_strength_function_mb", table=table, include_talys=True, plot_type="log-log", energy_interval=(0.1, 10), ylabel="$R$ %s" % units["strength_function_mb"])
    ...
    plot_table(column="cross_section", talys_column="cross_section_talys", table=table, include_talys=True,
               plot_type="log-log", energy_interval=(0, 20), ylabel="Cross-Section %s" % units["cross_section"])
