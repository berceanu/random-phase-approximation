...
//	if(rmeshorig[i] > 6.5)
	...
	cout << "program is terminating" << endl;
	cout << "Transition-energy in ztes_start.dat = " << tc_erg << endl;
	exit(1);
    }
    ...
    jtotfile.open("ztes_t_jtotal.out", ios::out | ios::trunc);    
    ...
    descfile.open("ztes_t_desc.out", ios::out | ios::trunc);    
    write_head(descfile,0,hp);
    descfile << "#Transitioncurrents for " << erpa[tr_st_ind] << " MeV" 
      << endl;
    descfile << "# z \t rho \t j_z \t j_rho" << endl;

    cur_tot_file.open("ztes_t_cur_tot.out", ios::out | ios::trunc);    
    cur_tot_file.setf(ios::scientific);

    vel_tot_file.open("ztes_t_vel_tot.out", ios::out | ios::trunc);    
    vel_tot_file.setf(ios::scientific);

    cur_neutron_file.open("ztes_t_cur_neutron.out", ios::out | ios::trunc);    
    cur_neutron_file.setf(ios::scientific);
    
    vel_neutron_file.open("ztes_t_vel_neutron.out", ios::out | ios::trunc);    
    vel_neutron_file.setf(ios::scientific);

    
    cur_proton_file.open("ztes_t_cur_proton.out", ios::out | ios::trunc);    
    cur_proton_file.setf(ios::scientific);
    
    vel_proton_file.open("ztes_t_vel_proton.out", ios::out | ios::trunc);    
