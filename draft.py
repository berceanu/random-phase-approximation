...
def log_fn(process):
    return (
        lambda line: logging.info(
            "{}: {:.2f}%\n".format(process, float(line.split()[-1]))
        )
        if "pa1 = " in line
        else None
    )
...
subfolders = (
    ...
    for parent in ("zero_temperature", "finite_temperature")
    for child in ("ground_state", "excited_states")
)
ztgs_dir, ztes_dir, ftgs_dir, ftes_dir = (
    os.path.join(base_path, subf) for subf in subfolders
)
...
if __name__ == "__main__":
    ...
    procs = [
        subprocess.Popen(
            ("make", "clean"), stdout=FNULL, stderr=subprocess.STDOUT, cwd=path
        )
    ...
    procs = [
        subprocess.Popen(
            ("make", "run"), stdout=FNULL, stderr=subprocess.STDOUT, cwd=path
        )
        for path in (ztgs_dir, ztes_dir, ftgs_dir, ftes_dir)
    ]
    for proc in procs:
    ...
    procs = [
        subprocess.Popen("./run", stdout=FNULL, stderr=subprocess.STDOUT, cwd=path)
