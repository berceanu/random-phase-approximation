...
if __name__ == "__main__":
    ...
    common_estate_block = (
        ...
        "calc       =   0                    ! 1:calc mat and exc. 0:only exc.\n"
        "xyprint    =   0                    ! saves xy-matrices on disk 1:yes 0:no\n"
        ...
        "xyread     =   1                    ! x- and y read in for further calc.\n"
    ...
    with open(os.path.join('out_no_matrix', "ztes_start.dat"), "w") as f:
        f.write(common_estate_block)
    #
    with open(os.path.join('out_no_matrix', "ftes_start.dat"), "w") as f:
        f.write(common_estate_block)

    # create FORTRAN input files
    with open(os.path.join('out_no_matrix', "dish_dis.dat"), "w") as f:
        f.write(common_gstate_block)
        f.write(zero_temp_gstate_block)
    #
    with open(os.path.join('out_no_matrix', "skys_dis.dat"), "w") as f:
        f.write(common_gstate_block)
